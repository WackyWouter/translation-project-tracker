<script language="javascript" src="<?php echo base_url(); ?>/assets/vendors/js/vendor.bundle.base.js?token=<?php echo CONF_asset_version; ?>"></script>
<script language="javascript" src="<?php echo base_url(); ?>/assets/js/jquery-3.6.1.min.js?token=<?php echo CONF_asset_version; ?>"></script>
<script language="javascript" src="<?php echo base_url(); ?>/assets/vendors/datatables.net/jquery.dataTables.js?token=<?php echo CONF_asset_version; ?>"></script>
<script language="javascript" src="<?php echo base_url(); ?>/assets/vendors/datatables.net-bs4/dataTables.bootstrap4.js?token=<?php echo CONF_asset_version; ?>"></script>
<script language="javascript" src="<?php echo base_url(); ?>/assets/js/moment.min.js?token=<?php echo CONF_asset_version; ?>"></script>
<script language="javascript" src="<?php echo base_url(); ?>/assets/js/datatable-moment.js?token=<?php echo CONF_asset_version; ?>"></script>
<script language="javascript" src="<?php echo base_url(); ?>/assets/vendors/bootstrap-datepicker/bootstrap-datepicker.min.js?token=<?php echo CONF_asset_version; ?>"></script>
<script language="javascript" src="<?php echo base_url(); ?>/assets/js/alertify.min.js?token=<?php echo CONF_asset_version; ?>"></script>

<script language="javascript" src="<?php echo base_url(); ?>/assets/js/common.js?token=<?php echo CONF_asset_version; ?>"></script>